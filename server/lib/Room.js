...
class Room extends EventEmitter
{
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		{
				logger.info(`收到request -${request.method}`);
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				const findPeer = this._protooRoom.peers.find( roomPeer => roomPeer.id === peer.id );
				// Store client data into the protoo Peer data object.
				findPeer.data.joined = true;
				findPeer.data.displayName = displayName;
				findPeer.data.device = device;
				findPeer.data.rtpCapabilities = rtpCapabilities;
				findPeer.data.sctpCapabilities = sctpCapabilities;
				// logger.info(findPeer);
				...
				logger.info('获取到加入的values',joinedPeers.map(opper=> opper.data.producers.values()))
				...
				for (const joinedPeer of joinedPeers)
				{
					...
					logger.info('计划创建 consumer',peer.data.id);
					for (const producer of joinedPeer.data.producers.values())
					{
						logger.info('计划创建 consumer',peer.data.id);
						this._createConsumer(
	...
	{
		logger.info(`chuangjian创建data conssumer`);
